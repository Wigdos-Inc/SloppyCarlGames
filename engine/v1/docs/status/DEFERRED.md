...
- [ ] [2026-07-23] Entities-list `movement`/`velocity`/`attacks`/`animations` overrides on template-ref spawns are silently ignored (template values win) — deferred by ED, because v1 override scope was limited to id/spawnSurfaceId/rootTransform/hp/collisionOverride/dialogue; extend `ResolveEntitySource` merge coverage if games need them.
